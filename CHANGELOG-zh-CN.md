...
- OceanBase v4.2 提交/回滚按钮状态与实际事务状态不同步 [#2097](https://github.com/oceanbase/odc/issues/2097)
- 带有单行注释的 SQL 语句执行失败 [#2085](https://github.com/oceanbase/odc/issues/2085)
- 最后一个 SQL 命令若不以分隔符结束将导致偏移量计算错误 [#1970](https://github.com/oceanbase/odc/issues/1970)

结果集

- 结果集中同时修改多行数据时耗时较久 [#2007](https://github.com/oceanbase/odc/issues/2007)
- OceanBase MySQL 模式下 DATETIME 数据类型显示时精度丢失 [#1411](https://github.com/oceanbase/odc/issues/1411)

表对象

- 索引和约束视图中列名顺序不一致 [#1948](https://github.com/oceanbase/odc/issues/1948)
- 无法查看 MySQL v5.6 表详情 [#1635](https://github.com/oceanbase/odc/issues/1635)
- 无法查看 Sofa ODP 表详情 [#2043](https://github.com/oceanbase/odc/issues/2043)

无锁结构变更

- 当输入语句包含注释时，OSC 任务会因为语法异常而失败 [#1597](https://github.com/oceanbase/odc/issues/1597)

SQL检查

- 存在虚拟列时发生空指针异常 [#2031](https://github.com/oceanbase/odc/issues/2031)
- 通过 DROP CONSTRAINT 删除主键的操作未能检测到 [#1879](https://github.com/oceanbase/odc/issues/1879)
- 特定 ALTER 语句会触发的空指针异常 [#1865](https://github.com/oceanbase/odc/issues/1865)

分区计划

- 当模式或表名为小写时，无法执行分区 DDL [#2088](https://github.com/oceanbase/odc/issues/2088)

用户与权限

- 批量导入用户包含角色名称会失败 [#1908](https://github.com/oceanbase/odc/issues/1908)

数据安全

- 使用嵌套 CASE-WHEN 子句时数据掩码不一致 [#1410](https://github.com/oceanbase/odc/issues/1410)

系统集成

- 请求正文中包含中文内容时出现乱码 [#1625](https://github.com/oceanbase/odc/issues/1625)

DB Browser

- 包含索引的表创建语句未被识别为创建类型 [#2063](https://github.com/oceanbase/odc/issues/2063)
- 在 Oracle 模式中 DDL 前后多余空格 [#2050](https://github.com/oceanbase/odc/issues/2050)
- 在 Oracle 11g 模式下无法检索有默认值列的表 [#1733](https://github.com/oceanbase/odc/issues/1733)
- listTables 未能一致地准确返回指定模式下的表 [#1632](https://github.com/oceanbase/odc/issues/1632)
- 在 OceanBase 版本 < V2.2.30 时无法 listTables [#1478](https://github.com/oceanbase/odc/issues/1478)
- MySQL 表结构设计的可视化不足，特别是对于单引号括起的字符串 [#1401](https://github.com/oceanbase/odc/issues/1401)

OB-SQL-Parser

- 解析命名为 'json_table' 的表的 INSERT 语句出错 [#1968](https://github.com/oceanbase/odc/issues/1968)

## 4.2.3_bp1 (2024-02-01)

### 功能变化

- 数据库变更：数据库变更任务适配流式读取 SQL 文件 [#1437](https://github.com/oceanbase/odc/issues/1437)
- 数据归档/清理：支持使用唯一索引进行分片 [#1327](https://github.com/oceanbase/odc/issues/1327)

### 缺陷修复

SQL 执行

- 在 OceanBase v2.2.30 上 执行 SQL 失败 [#1487](https://github.com/oceanbase/odc/issues/1487)
- 在团队空间执行匿名块导致 NPE [#1474](https://github.com/oceanbase/odc/issues/1474)
- 个人设置启用手动提交时，回滚操作没有成功执行 [#1468](https://github.com/oceanbase/odc/issues/1468)
- 不能设置超过 2 个字符的分隔符 [#1414](https://github.com/oceanbase/odc/issues/1414)
...
- 查询表数据时没有列注释 [#1488](https://github.com/oceanbase/odc/issues/1488)

数据导出

- 导出包体和同义词时，不显示对象类型。 [#1464](https://github.com/oceanbase/odc/issues/1464)

任务

- 创建没有连接信息的工单时 NPE [#1479](https://github.com/oceanbase/odc/issues/1479)
- 并发创建任务时无法正确设置任务状态 [#1419](https://github.com/oceanbase/odc/issues/1419)
- 当任务创建者和审批者不是当前用户时，查看任务审批节点时发生错误。

无锁结构变更

- OSC 任务在全量迁移完成时不显示手动交换表名 [#1357](https://github.com/oceanbase/odc/issues/1357)

数据库变更

- 部分场景查询任务详情抛出流程实例未找到异常 [#1325](https://github.com/oceanbase/odc/issues/1325)
- 任务日志过期时查询任务详情抛出文件未找到异常 [#1316](https://github.com/oceanbase/odc/issues/1316)

分区计划

- 如果关联的调度配置不存在，删除作业失败 [#1495](https://github.com/oceanbase/odc/issues/1495)

数据归档/清理

- 编辑速率限制配置后，数据清理任务调度失败 [#1438](https://github.com/oceanbase/odc/issues/1438)
...
- 连接到 OBOracle 的小写模式时，SQL 检查失败 [#1341](https://github.com/oceanbase/odc/issues/1341)
- MetaDB 默认字符集为 GBK 时，执行带有生僻字的 SQL 语句失败 [#1486](https://github.com/oceanbase/odc/issues/1486)

### 安全加固

- 升级 aliyun-oss-sdk 版本 [#1393](https://github.com/oceanbase/odc/issues/1393)
- 无锁结构变更在手动交换表时存在水平越权访问数据权限 [#1405](https://github.com/oceanbase/odc/issues/1405)
