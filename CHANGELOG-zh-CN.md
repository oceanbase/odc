...
- 部分场景会话在过期后未能删除，潜在导致资源泄漏 (#2125)
- 在高负载下 SingleConnectionDataSource.getConnection() 出现并发问题 (#1914)
- 即便用户名更正后，仍然出现OceanBase Oracle连接失败 (#1797)
- 尝试打开SQL控制台时偶遇404错误 (#1809)

SQL 执行

- OceanBase 4.2中提交/回滚按钮状态与实际事务状态不同步 (#2097)
- 带有单行注释的SQL语句执行失败 (#2085)
- 最后一个SQL命令若不以分隔符结束将导致偏移量计算错误 (#1970)
- 匿名块执行模块与Oracle 11g不兼容 (#1759)
- 时间戳类型显示时精度丢失 (#1411)
...
- 无法查看MySQL 5.6版本的表详细信息 (#1635)
- 无法访问Sofa ODP表的详细视图 (#2043)

无锁结构变更

- 当输入语句包含注释时，OSC任务遇到语法异常 (#1597)

SQL检查

- 虚拟列存在时发生空指针异常 (#2031)
- 作为约束删除主键的操作未能检测到 (#1879)
- 特定ALTER语句触发的空指针异常 (#1865)

分区计划

- 当模式或表名为小写时，无法执行分区DDL (#2088)
...
- 在Oracle模式中DDL前后多余空格 (#2050)
- 在Oracle 11g 模式下无法检索有默认值列的表 (#1733)
- listTables 未能一致地准确返回指定模式下的表 (#1632)
- 在OceanBase版本不晚于2.2.30时无法列出表 (#1478)
- MySQL表结构设计的可视化不足，特别是对于单引号括起的字符串 (#1401)

OB-SQL-Parser

- 解析命名为'json_table'的表的INSERT语句时出现问题 (#1968)
