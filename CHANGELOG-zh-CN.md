...
- Oracle 模式 schema 名、表名、列名大小写输入，行为和 PLSQL Developer 一致
...
- 支持对 OBOracle2OBOracle 链路配置归档任务
...
- SQL 窗口规范补充生产开启 PL 调试风险提示
- 新增 3 条 SQL 检查规则
    - 保留字不应作为对象名，启用该规则可识别数据库中使用保留字作为对象名的变更
    - offline 结构变更语句存在，启用该规则可识别数据库中 offline 的结构变更
    - 存在 truncate 语句，启用该规则可识别数据库中 trucate 语句的执行
- 数据库变更增加索引变更语句检测，如遇索引变更自动调整超时设置（120h）
- 新增消息通知
    - 支持的事件类型包括工单类型的审批状态和执行状态变化
...
- 在 OceanBase 2.2.30 上 执行 SQL 失败 (#1487)
- 在团队空间执行匿名块导致 NPE (#1474)
- 个人设置启用手动提交时，回滚操作没有成功执行 (#1468)
- 不能设置超过2个字符的分隔符 (#1414)
- 在SQL窗口查询请求期间，注销时前端崩溃。
...
- 连接到OBOracle的小写模式时，SQL Check 失败 (#1341)
