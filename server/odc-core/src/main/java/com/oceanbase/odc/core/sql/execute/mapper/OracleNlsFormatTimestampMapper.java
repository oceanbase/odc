...
/**
 * {@link OracleNlsFormatTimestampMapper}
 *
 * @author yh263208
 * @date 2023-07-04 21:02
 * @since ODC_release_4.2.0
 */
public class OracleNlsFormatTimestampMapper extends OracleGeneralTimestampMapper {

    private final OracleTimestampFormat format;

    public OracleNlsFormatTimestampMapper(@NonNull String pattern) {
    ...
    private Object map(CellData cellData) throws SQLException {
        Timestamp timestamp = cellData.getTimestamp();
        if (timestamp == null) {
            return null;
        }
        return new TimeFormatResult(format.format(timestamp), timestamp.getTime(), timestamp.getNanos(), null);
    }
