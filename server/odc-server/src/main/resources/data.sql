...
INSERT INTO config_system_configuration ( `key`, `value`, `description` ) VALUES( 'odc.task-framework.job-never-heart-after-started-seconds', '30', 'by check current jobs running status to controller next schedule, limit started job rate' ) ON DUPLICATE KEY UPDATE `id` = `id`;
INSERT INTO config_system_configuration ( `key`, `value`, `description` ) VALUES( 'odc.task-framework.job-never-heart-after-started-limit-count', '10', 'limit count of job never heart to odc server after started' ) ON DUPLICATE KEY UPDATE `id` = `id`;
