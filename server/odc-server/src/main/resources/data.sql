...
INSERT INTO config_system_configuration ( `key`, `value`, `description` ) VALUES( 'odc.task-framework.available-max-memory-for-running-job-in-mb', '0', 'available-max-memory-for-running-job-in-mb size MB, for controller schedule rate in process model' ) ON DUPLICATE KEY UPDATE `id` = `id`;
