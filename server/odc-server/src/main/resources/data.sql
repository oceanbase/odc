...
INSERT INTO config_system_configuration ( `key`, `value`, `description` ) VALUES( 'odc.task-framework.limit-running-job-total-memory-in-milli-bytes', '0', 'limit running job total memory in milli bytes, for controller schedule rate in process model' ) ON DUPLICATE KEY UPDATE `id` = `id`;
