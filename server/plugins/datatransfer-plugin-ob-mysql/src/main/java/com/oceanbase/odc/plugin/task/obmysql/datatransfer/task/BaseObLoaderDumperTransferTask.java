...
public abstract class BaseObLoaderDumperTransferTask<T extends BaseParameter> implements DataTransferTask {
    ...
    protected abstract void postHandle(TransferObjectsInfo result);

    @Override
    public TransferObjectsInfo getStatus() {
        TransferObjectsInfo status = new TransferObjectsInfo();
    ...
    @Override
    public TransferObjectsInfo transfer() throws Exception {
        try {
        ...
        TransferObjectsInfo status = getStatus();
        postHandle(status);
        return status;
    }
