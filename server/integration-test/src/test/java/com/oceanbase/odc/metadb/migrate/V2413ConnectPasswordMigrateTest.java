...
import com.oceanbase.odc.migrate.jdbc.common.V2413ConnectPasswordMigrate;

public class V2413ConnectPasswordMigrateTest extends ServiceTestEnv {
    ...
    public void migrate() {
        ...
        jdbcTemplate.update("INSERT INTO `odc_session_manager` ("
                ...
                + "`tenant`, `db_user`, `password`, `default_DBName`, `extend_info`, `salt`) "
                + " VALUES (1, '1', 'test1', 'OB_MYSQL', '127.1', '2881', '', 'mysql', 'root', '123456', 'mysql', '{}', '')");
        jdbcTemplate.update("INSERT INTO `odc_session_extended` ("
                + " `sid`, `session_timeout`, `sys_user`, `sys_user_password`) "
                + " VALUES ('1', '60', 'root', 'NjU0MzIx')");

        // execute
        V2413ConnectPasswordMigrate migrate = new V2413ConnectPasswordMigrate();
        migrate.migrate(dataSource);

        // verify
        int count = JdbcTestUtils.countRowsInTableWhere(jdbcTemplate, "odc_session_manager", "`password` <> '123456'");
