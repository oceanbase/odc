...
/**
 * This listener {@link ServiceTaskPendingListener} is executed when the task pending to update the
 * status of the {@link FlowTaskInstance}
 *
 * @author yh263208
 ...
 */
@Slf4j
public class ServiceTaskPendingListener extends BaseStatusModifyListener<FlowTaskInstance> {
    ...
    protected FlowNodeStatus doModifyStatusOnStart(FlowTaskInstance target) {
        flowInstanceRepository.updateStatusById(target.getFlowInstanceId(), FlowStatus.WAIT_FOR_EXECUTION);
        FlowNodeStatus status = FlowNodeStatus.PENDING;
