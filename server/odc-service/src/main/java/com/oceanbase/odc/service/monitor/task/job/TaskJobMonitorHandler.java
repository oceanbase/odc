...
public class TaskJobMonitorHandler implements MonitorEventHandler<TaskJobMonitorEvent> {

    @Autowired
    private MeterHolder meterHolder;

    @Override
    public void handle(TaskJobMonitorEvent source) {
        MeterName meterName = source.getAction().getMeterName();
        MeterKey meterKey = MeterKey.ofMeter(meterName);
        meterHolder.getCounterHolder().increment(meterKey);
    }
}
