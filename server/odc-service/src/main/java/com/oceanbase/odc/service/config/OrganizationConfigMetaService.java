...
@Service
@SkipAuthorize("isolated by organization")
public class OrganizationConfigMetaService {
    private final Map<String, ConfigurationMeta> keyToConfigMetaMap = new HashMap<>();

    @PostConstruct
    public void init() {
        List<ConfigurationMeta> configurations =
                YamlUtils.fromYamlList("config-meta/organization-config-meta.yml", ConfigurationMeta.class);
        configurations.forEach(configuration -> keyToConfigMetaMap.put(configuration.getKey(), configuration));
    }
