...
/**
 * {@link ColumnCharsetExists}
 *
 * @author yh263208
 * @date 2023-06-26 16:41
 * @since ODC_release_4.2.0
 */
public class ColumnCharsetExists implements SqlCheckRule {

    @Override
    public SqlCheckRuleType getType() {
        return SqlCheckRuleType.COLUMN_CHARSET_EXISTS;
    }
    ...
    private List<CheckViolation> builds(String sql, int offset, Stream<ColumnDefinition> stream) {
        return stream.filter(d -> {
            ...
            if (dataType instanceof CharacterType) {
                return ((CharacterType) dataType).getCharset() != null;
            } else if (dataType instanceof CollectionType) {
                return ((CollectionType) dataType).getCharset() != null;
            }
            return false;
