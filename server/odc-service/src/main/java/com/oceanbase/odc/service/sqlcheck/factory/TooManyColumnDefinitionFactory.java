...
public class TooManyColumnDefinitionFactory implements SqlCheckRuleFactory {
    ...
    public SqlCheckRule generate(@NonNull DialectType dialectType, Map<String, Object> parameters) {
        String key = getParameterNameKey("max-column-definition-count");
        if (parameters == null || parameters.isEmpty() || parameters.get(key) == null) {
            return new TooManyColumnDefinition(100);
        }
        return new TooManyColumnDefinition(Integer.valueOf(parameters.get(key).toString()));
    }
