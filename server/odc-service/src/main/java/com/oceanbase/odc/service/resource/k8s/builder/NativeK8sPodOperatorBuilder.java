...
public class NativeK8sPodOperatorBuilder extends BaseNativeK8sResourceOperatorBuilder<K8sPod> {
    ...
    @Override
    protected K8sPod newInstanceByEntity(ResourceEntity resourceEntity) {
        return fullFillByEntity(new K8sPod(), resourceEntity);
    }

    @Override
    protected K8sPod fullFillByEntity(K8sPod resource, ResourceEntity e) {
        resource.setResourceState(e.getStatus());
        resource.setResourceLocation(new ResourceLocation(e.getRegion(), e.getGroupName()));
        return resource;
    }
