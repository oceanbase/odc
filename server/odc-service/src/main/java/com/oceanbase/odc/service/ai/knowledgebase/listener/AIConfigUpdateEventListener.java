 ...
 */
@Component
public class AIConfigUpdateEventListener {
    @Autowired(required = false)
    private SchemaKBBuildManager schemaKBBuildManager;

    @EventListener(AIConfigUpdateEvent.class)
    public void onEvent(AIConfigUpdateEvent event) {
        if (event.getSource() == null) {
            return;
        }
        AIConfig aiConfig = (AIConfig) event.getSource();
        schemaKBBuildManager.submitBuildSchemaKBTaskForOrganization(aiConfig.getOrganizationId(), aiConfig);
    }
