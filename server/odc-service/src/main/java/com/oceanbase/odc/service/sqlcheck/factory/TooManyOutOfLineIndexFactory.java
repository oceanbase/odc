...
public class TooManyOutOfLineIndexFactory implements SqlCheckRuleFactory {
    ...
    public SqlCheckRule generate(@NonNull DialectType dialectType, Map<String, Object> parameters) {
        String key = getParameterNameKey("max-index-count");
        if (parameters == null || parameters.isEmpty() || parameters.get(key) == null) {
            return new TooManyOutOfLineIndex(10);
        }
        return new TooManyOutOfLineIndex(Integer.valueOf(parameters.get(key).toString()));
    }
