...
public class RestrictTableCharsetFactory implements SqlCheckRuleFactory {
    ...
    public SqlCheckRule generate(@NonNull DialectType dialectType, Map<String, Object> parameters) {
        String key = getParameterNameKey("allowed-charsets");
        Set<String> allowCharsets;
        if (parameters == null || parameters.isEmpty() || parameters.get(key) == null) {
            allowCharsets = new HashSet<>();
        } else {
            allowCharsets = new HashSet<>((List<String>) parameters.get(key));
        }
        return (dialectType.isMysql() || dialectType.isDoris()) ? new MySQLRestrictTableCharset(allowCharsets) : null;
    }
