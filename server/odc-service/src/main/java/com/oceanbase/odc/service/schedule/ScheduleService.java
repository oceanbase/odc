...
public class ScheduleService {
    ...
    public void changeSchedule(ScheduleChangeReq req) {
        ...
        ScheduleChangeLog changeLog = scheduleChangeLogService.createChangeLog(
                ScheduleChangeLog.build(targetSchedule.getId(), req.getOperationType(), targetSchedule.getParameters(),
                        JsonUtils.toJson(req.getParameters()), approvalFlowInstanceId));
        log.info("Create change log success,changLog={}", changeLog);

    }

    public void executeChangeSchedule(ScheduleChangeReq req) {
        Schedule targetSchedule = nullSafeGetById(req.getScheduleId());
        ...
        switch (req.getOperationType()) {
            ...
            case UPDATE: {
                scheduleRepository.updateJobParametersById(req.getScheduleId(),
                        JsonUtils.toJson(req.getParameters()));
                scheduleRepository.updateStatusById(targetSchedule.getId(), ScheduleStatus.ENABLED);
                break;
            }
            case PAUSE: {
    ...
    // TODO delete it
    public Schedule create(Schedule req) {
        ScheduleEntity entity = new ScheduleEntity();

        entity.setProjectId(1L);
        entity.setDescription(req.getDescription());
        entity.setJobParametersJson(JsonUtils.toJson(req.getParameters()));
        entity.setTriggerConfigJson(req.getTriggerConfigJson());
        ...
        // TODO target database info the schedule belongs to.
        entity.setDatabaseId(-1L);
        entity.setDatabaseName("deprecated");
        entity.setConnectionId(-1L);

        entity = scheduleRepository.save(entity);
        return scheduleMapper.entityToModel(entity);
    }

    private Schedule create(ScheduleChangeReq req) {
        ...
        entity.setDescription(req.getDescription());
        entity.setJobParametersJson(JsonUtils.toJson(req.getParameters()));
        entity.setTriggerConfigJson(JsonUtils.toJson(req.getTriggerConfig()));
        entity.setScheduleType(req.getType());
    ...
    public void innerUpdateTriggerData(Long scheduleId, Map<String, Object> triggerDataMap)
        ...
        Schedule scheduleConfig = nullSafeGetById(scheduleId);
    ...
    public void enable(Schedule schedule) throws SchedulerException, ClassNotFoundException {
        quartzJobService.createJob(buildCreateJobReq(schedule));
        scheduleRepository.updateStatusById(schedule.getId(), ScheduleStatus.ENABLED);
    }
    ...
    public void pause(Long scheduleId) {
        changeSchedule(ScheduleChangeReq.with(scheduleId, OperationType.PAUSE));

    }

    @Transactional(rollbackFor = Exception.class)
    public void resume(Long scheduleId) {
        changeSchedule(ScheduleChangeReq.with(scheduleId, OperationType.RESUME));
    }
    ...
    public void refreshScheduleStatus(Long scheduleId) {
        Schedule schedule = nullSafeGetById(scheduleId);
        JobKey key = QuartzKeyGenerator.generateJobKey(schedule);
        ...
        log.info("Update schedule status from {} to {} success,scheduleId={}}", schedule.getStatus(), status,
                scheduleId);
    }
    ...
    public Schedule nullSafeGetByIdWithCheckPermission(Long id, boolean isWrite) {
        Schedule schedule = nullSafeGetById(id);
    ...
    public Schedule nullSafeGetById(Long id) {
        Optional<ScheduleEntity> scheduleEntityOptional = scheduleRepository.findById(id);
        return scheduleMapper.entityToModel(
                scheduleEntityOptional.orElseThrow(() -> new NotFoundException(ResourceType.ODC_SCHEDULE, "id", id)));
    }


    private Trigger nullSafeGetScheduleTrigger(Schedule schedule) throws SchedulerException {
    ...
    private CreateQuartzJobReq buildCreateJobReq(Schedule schedule) {
        ...
        if (schedule.getScheduleType() == ScheduleType.ONLINE_SCHEMA_CHANGE_COMPLETE) {
            createQuartzJobReq.getJobDataMap().putAll(JsonUtils.fromJson(schedule.getParameters(), Map.class));
        } else {
    ...
    private Trigger getScheduleTrigger(Schedule schedule) throws SchedulerException {
