...
/**
 * Execute the listener when the task execution times out and set the task status to expired
 *
 * @author yh263208
 ...
 */
@Slf4j
public class ServiceTaskExpiredListener extends BaseStatusModifyListener<FlowTaskInstance> {
    ...
    protected FlowNodeStatus doModifyStatusOnEnd(FlowTaskInstance target) {
        flowInstanceRepository.updateStatusById(target.getFlowInstanceId(), FlowStatus.EXECUTION_EXPIRED);
