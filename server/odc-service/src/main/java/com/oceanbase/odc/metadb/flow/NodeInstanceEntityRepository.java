...
import com.oceanbase.odc.config.druid.DruidMonitor;
...
public interface NodeInstanceEntityRepository extends OdcJpaRepository<NodeInstanceEntity, Long>,
    ...
    default void batchCreate(List<NodeInstanceEntity> nodeInstanceEntity) {
        String flowInstanceNode = InsertSqlTemplateBuilder.from("flow_instance_node")
                ...
                .build();
        List<Object[]> params = nodeInstanceEntity.stream().map(
                t -> new Object[] {t.getInstanceId(), t.getInstanceType().name(), t.getFlowInstanceId(),
                        t.getActivityId(),
                        t.getName(), t.getFlowableElementType().name()})
                .collect(Collectors.toList());

        getJdbcTemplate().batchUpdate(flowInstanceNode, params);
    }
}
