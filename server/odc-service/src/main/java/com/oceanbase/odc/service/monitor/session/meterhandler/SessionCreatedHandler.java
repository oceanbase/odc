...
class SessionCreatedHandler extends AbstractHandler {
    ...
    void doHandle(SessionMonitorContext source) {
        if (source.getAction().equals(SessionAction.CREATED_SUCCESS)) {
            meterHolder.getCounterHolder().increment(MeterKey.ofMeter(MeterName.CONNECT_SESSION_TOTAL,
                    Tag.of("organizationId", String.valueOf(authenticationFacade.currentOrganizationId()))));
        }
        if (source.getAction().equals(SessionAction.EXPIRED_FAILED)) {
            meterHolder.getCounterHolder().increment(MeterKey.ofMeter(MeterName.CONNECT_SESSION_CREATED_FAILED_COUNT,
                    Tag.of("organizationId", String.valueOf(authenticationFacade.currentOrganizationId()))));
