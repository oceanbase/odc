...
/**
 * 
 * @description: all tests for {@link OBMySQLMViewEditor}
 * @author: zijia.cj
 * @date: 2025/4/2 21:26
 * @since: 4.3.4
 */
public class OBMySQLMViewEditorTest {

    private DBMViewEditor mViewEditor;

    @Before
    public void setUp() {
        mViewEditor = new OBMySQLMViewEditor(new OBMySQLSpecialDropIndexEditor());
    }
    ...
    public void generateUpdateObjectDDL_dropIndex_generateSucceed() {
        ...
        Assert.assertEquals("DROP INDEX `old_index` ON `whatever_schema`.`whatever_table`;\n", ddl);
    }
    ...
    public void generateUpdateObjectDDL_createIndex_generateSucceed() {
        ...
        Assert.assertEquals(
                "CREATE  INDEX `new_index` USING HASH ON `whatever_schema`.`whatever_table` (`col1`, `col2`) GLOBAL  COMMENT 'whatever_comment';\n",
                ddl);
    }
    ...
    public void generateUpdateObjectDDL_updateNameIndex_generateSucceed() {
        ...
        Assert.assertEquals("ALTER TABLE `whatever_schema`.`whatever_table` RENAME INDEX `old_index` TO `new_index`;\n",
                ddl);
    }
