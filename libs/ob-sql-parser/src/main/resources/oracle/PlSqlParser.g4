...
options {
    superClass = BasePLSQLParser;
    tokenVocab=PlSqlLexer;
}

@parser::header {
import com.oceanbase.tools.sqlparser.BasePLSQLParser;
}
...
subprog_decl_in_type
    : (MEMBER | STATIC) (proc_decl_in_type | func_decl_in_type)
...
procedure_call[boolean in_stmt]
    : {$in_stmt}? routine_name function_argument?
    | {!$in_stmt}? CALL routine_name function_argument?
    ;
...
concatenation
    ...
    | concatenation MOD concatenation
...
json_function
    ...
    | JSON_SERIALIZE '(' CHAR_STRING (RETURNING json_query_return_type)? PRETTY? ASCII? TRUNCATE? (
        (NULL_ | ERROR | EMPTY (ARRAY | OBJECT)) ON ERROR
    )? ')'
    ...
    | JSON_VALUE '(' expression (FORMAT JSON)? (
        ',' CHAR_STRING? json_value_return_clause? ((ERROR | NULL_ | DEFAULT literal)? ON ERROR)? (
            (ERROR | NULL_ | DEFAULT literal)? ON EMPTY
