 ...
 * @description:
 * @author: zijia.cj
 * @date: 2025/3/18 00:00
 * @since: 4.3.4
 */
public class MySQLCreateMaterializedViewOptsFactory extends OBParserBaseVisitor<CreateMaterializedViewOpts> implements StatementFactory<CreateMaterializedViewOpts> {
    ...
    @Override public CreateMaterializedViewOpts visitCreate_mview_opts(OBParser.Create_mview_optsContext ctx) {
...
//        ctx.mview_refresh_opt()
        if(Objects.nonNull(ctx.REWRITE())&&Objects.nonNull(ctx.COMPUTATION())){
            if(ctx.REWRITE().getSymbol().getStartIndex()<ctx.COMPUTATION().getSymbol().getStartIndex()){
                ...
                createMaterializedViewOpts.setEnableQueryComputation(Objects.nonNull(ctx.mview_enable_disable(1).ENABLE()));
            }else {
                createMaterializedViewOpts.setEnableQueryComputation(Objects.nonNull(ctx.mview_enable_disable(0).ENABLE()));
